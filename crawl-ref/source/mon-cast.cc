...
/**
 * Have a monster attempt to cast corrupt locale.
 * Currently only used by mlioglotl.
 * The author is aware that some players might try
 * to drag mlioglotl around the dungeon such that this
 * spell can be used to break open level geometry.
 * A few proposed solutions:
 * 1. Make Mlioglotl fast (currently implemented option)
 * 2. Prevent Mlioglotl climbing stairs 
 * 3. Add some extra logic to monster version of corrupt
 *    such that walls are never broken
 *    (nuclear option, may lose flavour)
